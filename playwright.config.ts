...
export default defineConfig({
  ...
  projects: [
    ...
    { name: 'chromium', use: { ...devices['Desktop Chrome'] }, testIgnore: /(?:yfinance(?:-(?:ui-253|mobile|templates|indicator-source|routed-study))?|widget-ui-253|widget-data-loading|widget-localization|widget-alerts|chart-data-export|instruments|indicator-source-markers|alert-line-drag|table-layout)\.spec\.ts/ },
    ...(['chromium', 'firefox', 'webkit'] as const).map(browserName => ({
      name: `widget-loading-${browserName}`,
      testMatch: /(?:widget-ui-253|widget-data-loading|widget-localization|widget-alerts|drawing-future|drawing-catalog|analysis-linked-events|widget-objects|navigation-wheel|widget-mobile|branding-watermark|crosshair-snap|workspace-storage|open-interest|alerts|alert-line-drag|table-layout|replay-time|chart-data-export|instruments|indicator-source-markers|indicator-visuals|scale-state|native-timeframe-navigation|native-fill-gradients|native-series-scale-assignment|native-indicator-lifecycle|native-study-scale-assignment|native-requested-provider|native-external-lifecycle|native-indicator-alerts|native-study-dependencies|native-study-source-ui|native-multiple-price-axes|native-plot-scale-assignments|native-template-layouts|native-numerical-indicators|native-chart-preferences|native-typed-inputs|native-output-targets|indicator-curved-polylines|table-cell-tooltips|indicator-window-arithmetic|cpr-period-observations|user-navigation-policy(?:-reference)?|indicator-text-style)\.spec\.ts/,
