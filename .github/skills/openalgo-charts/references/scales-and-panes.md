...
The first legend row of a lower pane carries a `collapse` action (`PaneLegendAction`) between `down` and `maximize`; its glyph is a header bar over a chevron that points up while the pane is open and down once it is folded (`PaneLegendOptions.collapsed`). This is separate from `setIndicatorLegendCollapsed`, which hides study legend rows behind a count and never changes a pane. With both on, a strip keeps its first study row, because its collapse control is the only way back on the canvas; the row folds behind the count again once the pane opens.
