...
| `undo()` / `redo()` / `canUndo()` / `canRedo()` | History. A step left with nothing to do once a drawing in it is made read-only is dropped, so `canUndo()` and `canRedo()` report only a press that changes something. |
...
`{ force: true }`. A drawing belongs to
