...
| `loadMore(until?)` | Share a pending page, deduplicate older bars and reject obsolete responses after a context change. `until` (UTC seconds) widens the first date window to reach that time in one request; a paged feed still returns one `countBack` page per call. |
